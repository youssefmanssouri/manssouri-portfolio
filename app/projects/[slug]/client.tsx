...
export function ProjectCaseStudyClient({ project, nextProject }: ProjectCaseStudyClientProps) {
  ...
  const handleOpenLightboxBySrc = (src: string) => {
    if (!src || allImages.length === 0) return;
        ...
        <section id="overview" className="scroll-mt-36 space-y-12 sm:space-y-16">
          ...
          <div className="space-y-4 pb-8 border-b border-[#DED6CC]">
            ...
            <div className="flex flex-wrap items-center gap-3 pt-3">
              ...
              {project.hasLiveDemo && project.liveUrl && (
                ...
                >
                  <span>{isFr ? (project.liveUrlLabelFr || t("work.visitLive")) : (project.liveUrlLabelEn || t("work.visitLive"))}</span>
              ...
              {project.downloadUrl && (
                <a
                  ...
                  download
                  onClick={() => {
                    trackEvent("DOWNLOAD_APK_CLICK", { slug: project.slug, name: project.name, source: "case_study_top" });
                  }}
                  className="inline-flex items-center gap-2 bg-[#3A171C] text-[#F3EFEA] px-5 py-2.5 rounded-xs text-xs font-semibold uppercase tracking-wider hover:bg-[#2D1216] transition-all active:scale-[0.98] shadow-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#A65F4B]"
                >
                  <span>{isFr ? (project.downloadLabelFr || "Télécharger l'aperçu Android") : (project.downloadLabelEn || "Download Android Preview")}</span>
                  <ExternalLink className="w-3.5 h-3.5" />
                </a>
              )}
          ...
          <div className="space-y-2">
            {project.heroImage ? (
              ...
              <div className="w-full relative aspect-[16/10] sm:aspect-[16/9] rounded-xs overflow-hidden border border-[#DED6CC]/30 bg-[#2D1216] text-[#F3EFEA] shadow-lg flex flex-col justify-between p-6 sm:p-10 lg:p-14">
                <div className="flex items-center justify-between border-b border-[#DED6CC]/20 pb-4">
                  <div className="flex items-center gap-2 text-xs font-mono text-[#A65F4B] uppercase tracking-widest font-bold">
                    <span>{category}</span>
                    <span>·</span>
                    <span>ARCHITECTURAL CASE STUDY</span>
                  </div>
                  <span className="text-[11px] font-mono text-[#DED6CC]/60 bg-[#F3EFEA]/5 px-2.5 py-1 rounded-xs border border-[#DED6CC]/15">
                    {isFr ? "Captures réelles en cours de préparation" : "Real device captures pending"}
                  </span>
                </div>

                <div className="max-w-2xl space-y-3 py-6">
                  <span className="text-xs font-mono text-[#A65F4B] uppercase tracking-widest font-semibold block">
                    {isFr ? "Aperçu de l'Application Mobile" : "Mobile Application Overview"}
                  </span>
                  <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-[#F3EFEA] tracking-tight">
                    {project.title}
                  </h2>
                  <p className="text-xs sm:text-sm text-[#DED6CC]/80 leading-relaxed max-w-xl">
                    {isFr ? project.shortDescriptionFr : project.shortDescription}
                  </p>
                </div>

                <div className="pt-4 border-t border-[#DED6CC]/20 flex flex-wrap items-center justify-between gap-4 text-xs font-mono text-[#DED6CC]/70">
                  <div className="flex flex-wrap gap-2">
                    {project.technologies.slice(0, 5).map((tech) => (
                      <span key={tech} className="px-2 py-0.5 rounded-xs bg-[#F3EFEA]/5 border border-[#DED6CC]/15 text-[#F3EFEA]">
                        {tech}
                      </span>
                    ))}
                  </div>
                  <div className="text-[11px] text-[#DED6CC]/50">
                    {isFr ? "Aperçu Android Build 10 disponible ci-dessous" : "Android Build 10 preview available below"}
                  </div>
                </div>
              </div>
            )}
          ...
          {project.hasLiveDemo && project.liveUrl && (
            <div className="p-8 sm:p-10 rounded-xs bg-[#FAF7F2] border border-[#A65F4B]/50 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="max-w-xl space-y-2">
                ...
                {project.downloadNoteEn && (
                  <p className="text-[11px] font-mono text-[#242222]/60 pt-1">
                    ℹ️ {isFr ? project.downloadNoteFr : project.downloadNoteEn}
                  </p>
                )}
              </div>
              ...
              <div className="shrink-0 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                ...
                {project.downloadUrl && (
                  <a
                    ...
                    download
                    onClick={() => {
                      trackEvent("DOWNLOAD_APK_CLICK", { slug: project.slug, name: project.name, source: "case_study_sandbox_banner" });
                    }}
                    className="inline-flex items-center justify-center gap-2 bg-[#3A171C] text-[#F3EFEA] px-6 py-3 rounded-xs text-xs font-semibold uppercase tracking-wider hover:bg-[#2D1216] transition-all active:scale-[0.98] shadow-xs"
                  >
                    <span>{isFr ? (project.downloadLabelFr || "Télécharger l'aperçu Android") : (project.downloadLabelEn || "Download Android Preview")}</span>
                    <ExternalLink className="w-4 h-4" />
                  </a>
